...
import torch
from meta_components import RDB
import getopt
import math
import sys
import torch
...
class ReconNet(nn.Module):
	def __init__(self):
		...
		self.D, C, G = {'A': (20, 6, 32),'B': (16, 8, 64),}['B']

		# Shallow feature extraction net
		self.SFENet1 = nn.Conv2d(G0, G0, kSize, padding=(kSize - 1) // 2, stride=1)
...
class HR_tail(nn.Module):
	...
	def forward(self, feats):
		feats = self.lrelu(self.pixel_shuffle(self.upconv1(feats)))
		out = self.lrelu(self.pixel_shuffle(self.upconv2(feats)))
...
class VISRNet(nn.Module):
	...
	def forward(self, frame1, frame3):
		...
		reconstructed_features=self.RDB_reconstruct(enhanced_features)
		SR_output=self.super_resolution(reconstructed_features)
		return interpolated_frame,SR_output
